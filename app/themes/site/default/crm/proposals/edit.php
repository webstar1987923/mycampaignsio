...
<div class="ciuis-body-content">
				...
				<div class="col-md-12 md-m-0 md-p-0">
					<div class="form-group">
						...
						<div class="col-md-5 md-pl-0">
							...
							<?php if ($proposals['relation_type'] == 'customer') {
    echo '<option value="customer">'.lang('customer').'</option>';
}  ?>
							<?php if ($proposals['relation_type'] == 'lead') {
    echo '<option value="lead">'.lang('lead').'</option>';
						...
						<div class="col-md-5 md-pr-0 customer-select" style="<?php if ($proposals['relation_type'] == 'lead') {
    echo 'display:none';
}?>">
							...
							<select required name="related_customer" class="form-control select2">
							<?php if ($proposals['type']==0) {
    $pcustomer =  $proposals['customer'];
} else {
    $pcustomer = $proposals['individual'];
}?>
							<?php if ($proposals['relation_type'] == 'customer') {
    echo '<option value="'.$proposals['relation'].'">'.$pcustomer.'</option>';
						...
						<div class="col-md-5 md-pr-0 lead-select" style="<?php if ($proposals['relation_type'] == 'customer') {
                                    echo 'display:none';
                                }?>">
							<label for="leads">
								<?php echo lang('leads'); ?>
				...
				<div class="col-md-12 md-m-0 md-p-0">
					<div class="form-group">
						...
						<div class="col-md-6 md-pr-0">
							...
							<select required name="statusid" class="form-control select2">
								<?php if ($proposals['statusid'] == 1) {
                                    echo '<option value="1">'.lang('draft').'</option>';
                                }  ?>
								<?php if ($proposals['statusid'] == 2) {
                                    echo '<option value="2">'.lang('sent').'</option>';
                                }  ?>
								<?php if ($proposals['statusid'] == 3) {
                                    echo '<option value="3">'.lang('open').'</option>';
                                }  ?>
								<?php if ($proposals['statusid'] == 4) {
                                    echo '<option value="4">'.lang('revised').'</option>';
                                }  ?>
								<?php if ($proposals['statusid'] == 5) {
                                    echo '<option value="5">'.lang('declined').'</option>';
                                }  ?>
								<?php if ($proposals['statusid'] == 6) {
                                    echo '<option value="6">'.lang('accepted').'</option>';
                                }  ?>
			...
			<div class="col-md-6 md-pl-30">
				...
				<div class="form-group has-warning">
					...
					<div data-min-view="2" data-date-format="dd.mm.yyyy" class="input-group date datetimepicker"> <span class="input-group-addon btn btn-default"><i class="icon-th mdi mdi-calendar"></i></span>
						<input placeholder="<?php echo date(" d.m.Y "); ?>" type='input' name="opentill" value="<?php echo($this->input->post('opentill') ? $this->input->post('opentill') : _udate($proposals['opentill'])); ?>" class="form-control" id="opentill"/>
					</div>
				</div>
