...
class CampaignIo_Pages_Authorized extends CampaignIo_Pages {
    ...
    public function single_domain_e_commerce(){
        ...
        //     }
        // }
    // get_total_visits( $user_id, $domainId, $days = 30 )
        // $traffic = $new_analytics_handler->get_google_analytics_traffic_info_date_range( $user_id, $domain_id, date( 'Y-m-d', strtotime("-30 days")), date('Y-m-d') );
        // $total_traffic = 0;
        ...
        //     }
        // }
        $traffic = $new_analytics_handler->get_total_visits_date_range( $user_id, $domain_id, date('Y-m-01'), date('Y-m-d') );
        $total_traffic = 0;
        if($traffic['status'] == 'success') {
            foreach($traffic['payload']['totalVisitsGraph'] as $tr){
                $total_traffic+=$tr;
            }
        }
        ...
        $traffic_prev = $new_analytics_handler->get_total_visits_date_range( $user_id, $domain_id,  $month_ini->format('Y-m-d'), $month_end->format('Y-m-d') );
        $total_traffic_prev = 0;
        if($traffic_prev['status'] == 'success') {
            foreach($traffic_prev['payload']['totalVisitsGraph'] as $tr){
                $total_traffic_prev+=$tr;
            }
        }
        $this->page_data['ecom_data']['total_traffic_prev'] = $total_traffic_prev;
        

        $this->page_data['product_data'] = $new_analytics_handler->get_product_data( $user_id, $domain_data, $this->usergaaccount_model, $this->page_data['current_days_filter'], $domain_id );


        $current_sales_report = $this->analyze_model->get_woo_current_month_sales($domain_id);
        $last_month_sales_report = $this->analyze_model->get_woo_last_month_sales($domain_id);

        $this->page_data['ecom_data']['current_month'] = $current_sales_report;
        $this->page_data['ecom_data']['last_month'] = $last_month_sales_report;

        

        $transactions_data = $new_analytics_handler->get_transactions( $user_id, $domain_data, $this->usergaaccount_model, $this->page_data['current_days_filter'], $domain_id );
    ...
    public function single_domain_wordpress_unreachable(){
        $domain_id = (int) $this->uri->segment(2);
        $user_id = $this->page_data['user']['id'];
        ...
        if( ! isset( $_SESSION[ 'wp-site-login-unreachable-' . $domain_id ] ) || 1 !== $_SESSION[ 'wp-site-login-unreachable-' . $domain_id ] ){
            redirect( base_url('domains') );
            exit;
        }
        ...
        $this->page_data['try_login_url'] = base_url('domains/' . $domain_id . '/wordpress/login');
        $this->page_data['edit_domain_url'] = base_url('domains/' . $domain_id . '/edit');
        $this->page_data['domains_list_url'] = base_url('domains');

        $this->page_data['current_page'] = 'single-domain-wordpress-unreachable';
    ...
    public function single_domain_wordpress_update(){
        ...
        if( isset( $p['plugin'] ) ){
            $request['plugins'] = $p['plugin'];
        }

        if( isset( $p['theme'] ) ){
            $request['themes'] = $p['theme'];
        }

        if( isset( $p['core'] ) ){
            $request['core'] = (int) $p['core'] ? 1 : 0;
        }
    ...
    public function sub_users(){
    	$this->page_data['document_data']['title'] = 'Subusers - ' . $this->page_data['document_data']['title'];
        $this->display_page('sub-users');
    }

    public function sub_users_add(){
    	$this->page_data['document_data']['title'] = 'Add Subuser - ' . $this->page_data['document_data']['title'];
        $this->display_page('sub-users-add');
    }
    
    public function single_sub_user_edit(){
    	$this->page_data['document_data']['title'] = 'Edit Subuser - ' . $this->page_data['document_data']['title'];
        $this->display_page('single-sub-user-edit');
    }

    public function groups(){
    	$this->page_data['document_data']['title'] = 'Groups - ' . $this->page_data['document_data']['title'];
        $this->display_page('groups');
    }
    
    public function groups_add(){
    	$this->page_data['document_data']['title'] = 'Add Group - ' . $this->page_data['document_data']['title'];
        $this->display_page('groups-add');
    }
    
    public function single_group_edit(){
    	$this->page_data['document_data']['title'] = 'Edit Group - ' . $this->page_data['document_data']['title'];
        $this->display_page('single-group-edit');
    }

    public function business_overview(){
    	$this->page_data['document_data']['title'] = 'Business Overview - ' . $this->page_data['document_data']['title'];
        $this->display_page('business-overview');
    }
    ...
    //@note: insert woocommerce api data to db
    public function woo_api_hook($domainid)
    {

        //@note:  get domain name from domainid
        $this->db->select('domain_name, is_ecommerce, wooapi_consumer_key, wooapi_consumer_secret');
        $query = $this->db->get_where( 'domains', array('id' => $domainid ) );
        $domain_name = $query->result()[0]->domain_name;
        $is_ecommerce = $query->result()[0]->is_ecommerce;
        $wooapi_consumer_key = $query->result()[0]->wooapi_consumer_key;
        $wooapi_consumer_secret = $query->result()[0]->wooapi_consumer_secret;


        if ( $is_ecommerce!=0 && isset( $wooapi_consumer_key ) && isset( $wooapi_consumer_secret ) ):

            //@note:  pull woo api
            $woocommerce = new Client(
                // 'https://www.modiushealth.com', 
                // 'ck_50aa69c081961c4bf1e80467e0a203c4f01febe6', 
                // 'cs_da68d68414c21b7fc0b8af08c090fffff44078cf',
                $domain_name,
                $wooapi_consumer_key,
                $wooapi_consumer_secret,
                [
                    'wp_api' => true,
                    'version' => 'wc/v2', 
                    'timeout' => 100,
            ...
            foreach ($woo_customers as $customer) {
                ...
                $this->analyze_model->update_woo_customers( $customer_save_data );
            }

            $woo_orders = $woocommerce->get('orders');

            foreach ($woo_orders as $order) {
                $order_save_data = array(
                    'domain_id' => $domainid,
                    'order_id' => $order->id,
                    ...
                    'line_items' => serialize($order->line_items),
                    'refunds' => serialize($order->refunds),
                );

                $this->analyze_model->update_woo_orders( $order_save_data );
            }

            $woo_products = $woocommerce->get('products');

            foreach ($woo_products as $product) {
                $product_save_data = array(
                    'domain_id' => $domainid,
                ...
                $this->analyze_model->update_woo_products( $product_save_data );
            }


            $query = [
                'period' => 'last_month',
                // 'date_min' => '2018-01-01',  // Return sales for a specific start date, the date need to be in the YYYY-MM-DD format.
                // 'date_max' => '2018-01-18', //Return sales for a specific end date, the date need to be in the YYYY-MM-DD format.
            ];
            $woo_sales_report = $woocommerce->get('reports/sales', $query);
            
            foreach ($woo_sales_report as $report) {
                $report_save_data = array(
                    'domain_id' => $domainid,
                ...
                $this->analyze_model->update_woo_sales_report( $report_save_data );
            }
